...
export function Cart(){
    ...
    <div className="h-[85%] w-[85%] bg-custom-gradient shadow-[0px_10px_30px_5px_hsl(214.29_17.41%_40%)] rounded-lg p-10">
        <div className="font-dancingScript text-primary text-2xl font-extrabold hover:bg-gray-900 cursor-pointer">Amazing cart</div>
        <div className="flex gap-5">
            <div className="flex-[3] rounded gap-3">
                Your shopping list
                {cartItemState.map((item,index) => <ItemsInCart key={index} image={item.image} name={item.name} quantity={item.quantity}/>)}
            </div>
            <div className="flex-[1] p-4 bg-red-900 rounded">Checkout</div>
        </div>
    </div>
</div>
}